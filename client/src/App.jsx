...
import BulkUpload from './pages/BulkUpload.jsx';   // ← NEW
...
const ExpiredPage = () => (
  <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'var(--bg)' }}>
    <div className="rounded-2xl p-10 max-w-sm w-full text-center" style={{ background: 'var(--card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-card)' }}>
      ...
      <a href="/" className="inline-flex items-center justify-center gap-2 bg-indigo-500 text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-indigo-600 transition-colors">
        Go to ClickSphere
      </a>
    </div>
  </div>
  ...
  return (
    <AnimatePresence mode="wait">
      <Routes location={location} key={location.pathname}>
        ...
        <Route path="/dashboard/create"           element={<ProtectedRoute><CreateLink /></ProtectedRoute>} />
        <Route path="/dashboard/bulk"             element={<ProtectedRoute><BulkUpload /></ProtectedRoute>} />
        <Route path="/dashboard/analytics"        element={<ProtectedRoute><Analytics /></ProtectedRoute>} />
        <Route path="/dashboard/analytics/:urlId" element={<ProtectedRoute><Analytics /></ProtectedRoute>} />
